...
export default async function GalleryPage() {
  ...
  if (publicOrganization && !publicOrganization.settings.gallery_enabled) notFound();
  await logPageView("/gallery", publicOrganization?.organization.id);
  const supabase = await createClient();
  let query = supabase.from("images").select("*").is("deleted_at", null);
